...
export class Database {
  constructor(emissions_records, population_records) {
    ...
    this.huvudsektorer = this.emissions_records
      .map("Huvudsektor")
      .uniq()
      .sort()
      .value();
    ...
    this.kommuer_by_län = this.emissions_records.reduce((result, record) => {
      if (result[record.Län] == null) {
        result[record.Län] = new Set();
      }
  ...
  query({ filter, by_population }) {
    ...
    return result.reduce((result, values, key) => {
      ...
      years.forEach((year) => {
        ...
        if (by_population) {
          let population = this.populations_by_kommun_name[key].populations[
            year
          ];
  ...
  percentage_change(kommun) {
    ...
    let first_year_value = _(records)
        .map(years[0])
        .sum(),
      last_year_value = _(records)
        .map(years[years.length - 1])
        .sum();
    return this.round_number(
      (100.0 * (last_year_value - first_year_value)) / first_year_value
    );
  }
