<template>
  <div id="app">
    <Header @open="open_dialog($event)" />

    <DialogContent v-bind:is_open="dialog_is_open" @close="close_dialog()" />
      ...
      <div class="column is-one-quarter">
        <div id="filters">
          ...
          <div class="field">
            ...
            <div class="select is-fullwidth">
              <select id="kommun" v-model="selected_kommun">
                ...
                <option
                  v-for="kommun in all_kommuner_for(län)"
                  v-bind:key="kommun"
                >
                  {{ kommun }}
                </option>
          ...
          <div class="field toggle-buttons">
            <div class="buttons has-addons">
              <button
                ...
                :class="{
                  'is-selected': selected_value_type === 'change',
                  button: true,
                }"
              >
                Förändringstakt
              ...
              <button
                ...
                :class="{
                  'is-selected': selected_value_type === 'total',
                  button: true,
                }"
              >
                Totalutsläpp
          ...
          <div class="field toggle-buttons">
            <div class="buttons has-addons">
              <button
                ...
                :class="{
                  'is-selected': selected_emission_type === 'all',
                  button: true,
                }"
              >
                Växthusgaser
              ...
              <button
                ...
                :class="{
                  'is-selected': selected_emission_type === 'co2',
                  button: true,
                }"
              >
                Koldioxid
          ...
          <div class="sektor-buttons">
            <div
              v-for="(huvudsektor, index) in all_sektorer"
              v-bind:key="huvudsektor"
            >
              <input
                type="checkbox"
                class="is-sr-only"
                :disabled="
                  selected_value_type === 'total' &&
                  huvudsektor === industri_sektor
                "
                v-bind:id="'sektor-button-' + index"
                v-bind:value="huvudsektor"
                v-model="selected_huvudsektorer"
              />
          ...
          <div class="data-credits">
            <p>
              Data från
              <a
                href="http://extra.lansstyrelsen.se/rus/Sv/statistik-och-data/nationell-emissionsdatabas/Pages/default.aspx"
                >RUS</a
              >
              och
              <a
                href="http://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0101__BE0101C/BefArealTathetKon/?rxid=bd5169ae-f630-42db-8c8e-3ffdbf806a73"
                >SCB</a
              >
            </p>
            <p>
...
const population_data_by_kommun = _.reduce(
  populations_data,
  (result, record) => {
    result[record.kommun.name] = _.map(years, (year) => {
      return record.populations[year];
    });
...
export default {
  ...
  computed: {
    ...
    year_data_sets() {
      let co_equivalents_year_data = this.emissions_database
          .filter({ Ämne: "CO2-equivalents" })
          .year_data_by_kommun(population_data_by_kommun),
        co_year_data = this.emissions_database
          .filter({ Ämne: "CO2" })
          .year_data_by_kommun(population_data_by_kommun),
        percent_change_co2_equivalents = this.percent_year_over_year_change_data(
          this.emissions_database
            .filter({ Ämne: "CO2-equivalents" })
            .year_data_by_kommun()
        ),
        percent_change_co2 = this.percent_year_over_year_change_data(
          this.emissions_database.filter({ Ämne: "CO2" }).year_data_by_kommun()
        );

      let emission_points_fn = (emission, min_value, max_value) => {
      ...
      let result = [
        {
          ...
          description:
            "Årliga utsläpp av växthusgaser. (Samtliga växthusgaser totalt, per capita).",
          unit: "ton/invånare",
          data: co_equivalents_year_data,
          metrics: _.reduce(
            co_equivalents_year_data,
            (result, year_data, kommun) => {
        ...
        },
        {
          ...
          description:
            "Årligt utsläpp av koldioxid jämfört med startåret. (Koldioxid totalt, per capita).",
          unit: "ton/invånare",
          data: co_year_data,
          metrics: _.reduce(
            co_year_data,
        ...
        },
        {
          ...
          description:
            "Årligt utsläpp av växthusgaser jämfört med startåret. (Samtliga växthusgaser totalt, per capita).",
          unit: "procent",
          data: percent_change_co2_equivalents,
          metrics: _.reduce(
            this.mean_year_data(percent_change_co2_equivalents),
            (result, year_data, kommun) => {
              let rel_change = year_data[year_data.length - 1] / 100,
                yoy_change =
                  Math.pow(1 + rel_change, 1 / (year_data.length - 1)) - 1;
        ...
        },
        {
          ...
          description:
            "Årlig procentuell förändringstakt under mätperioden. (Koldioxid totalt, per capita).",
          unit: "procent",
          data: percent_change_co2,
          metrics: _.reduce(
            this.mean_year_data(percent_change_co2),
            (result, year_data, kommun) => {
              let rel_change = year_data[year_data.length - 1] / 100,
                yoy_change =
                  Math.pow(1 + rel_change, 1 / (year_data.length - 1)) - 1;
      ...
      _.each(result, (year_data_set) => {
        ...
        year_data_set.points = _.mapValues(
          year_data_set.metrics,
          (percentage) => {
  ...
  methods: {
    ...
    percent_year_over_year_change_data(year_data_by_kommun) {
      return _.reduce(
        year_data_by_kommun,
        (result, year_data, kommun) => {
          result[kommun] = [0].concat(
            _.map(_.range(1, year_data.length), (index) => {
              // TODO What should we do if the previous year is zero but the current year isn't?
              return (
                (100 * (year_data[index] - year_data[0])) /
                (year_data[0] + 0.00001)
              );
            })
          );
      ...
      return _.reduce(
        ...
        (result, year_data, kommun) => {
          ...
          result[kommun] = _.map(_.range(0, year_data.length), (index) => {
            return (
              year_data[year_data.length - 1] * (index / (year_data.length - 1))
            );
          });
...
$primary: #0080cc;
$primary: #f45959;
$family-primary: "Work Sans", BlinkMacSystemFont, -apple-system, "Segoe UI",
