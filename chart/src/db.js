...
export class EmissionsDatabase {
  constructor({ years, records }) {
    ...
    this.länen = _(records)
      .map("Län")
      .uniq()
      .sort()
      .value();
    this.kommuner = _(records)
      .map("Kommun")
      .uniq()
      .sort()
      .value();
    this.sektorer = _(records)
      .map("Huvudsektor")
      .uniq()
      .sort()
      .value();

    this.kommuner_by_län = _.reduce(
      records,
      (result, record) => {
    ...
    let r = _.reduce(
      ...
      (result, record) => {
        if (result[record.Kommun]) {
          result[record.Kommun] = _.map(
            result[record.Kommun],
            (year_sum, index) => {
